...
import * as auth0UserDetails from 'auth0-js';
...
export class AuthService {
	...
	auth0UserDetails = new auth0.WebAuth({
		domain: 'grceri-auth.auth0.com',
		clientID: '96b0aXJKxvJRK4gYKR0YuJnEnXN8cJsE'
	});
	...
	public signup(email: string, password: string, plan?: string, options?: object): Observable<string> {
		let a = Observable.create(observer => {
	...
	private setSession(authResult, renew?): void {
		...
		this.LS.set('user_id', authResult.idTokenPayload.sub.replace('auth0|', ''));
